...
const IterationForm = ({ onClose }) => {
  ...
  useEffect(() => {
    ...
    const fetchUserStories = async () => {
      try {
        ...
        // Extract user IDs from user stories and set the userStory state
        const extractedUserIDs = userStoriesData.map(story => story.assignees).flat();
        setUserStory(extractedUserIDs);
      } catch (error) {
  ...
  useEffect(() => {
    userStory.map((s) => {
      users.map((u) => {
        return u?._id === s ? setFilters(prevFilters => [...prevFilters, u]) : null;
      });
    });
  }, [userStory]);
